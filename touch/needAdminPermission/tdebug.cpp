...
TLOG_LEVEL TDebug::level = TLOG_VERBOSE;
QTextStream TDebug::logOut(stdout);
QFile TDebug::logFile("needAdminPremission.log");
TDebug::TDebug()
{
}
TDebug::~TDebug()
...
void TDebug::writeLogToFile(QString log)
{
    ...
    if (!TDebug::logFile.isOpen()) {
        ...
        if (TDebug::logFile.size() > 20 * 1024 * 1024) {
            QString toFile = "needAdminPremission_log_" + QDateTime::currentDateTime().toString("yyyy-MM-dd-hhmmss") + ".txt";
            bool result = TDebug::logFile.rename(toFile);
            if (result) {
                TDebug::logFile.close();
                TDebug::logFile.setFileName("needAdminPremission.log");
                TDebug::logFile.open(QIODevice::ReadWrite|QIODevice::Text|QIODevice::Append);
            } else {
    ...
    if (TDebug::logFile.isOpen()) {
        ...
        TDebug::logFile.write(infos.toStdString().c_str(), infos.length());
    } else {
        TDebug::logOut << infos.toStdString().c_str();
    }
...
void TDebug::debug(QString message)
{
    if (TDebug::level < TLOG_DEBUG)
        return;
    QString infos = QString("" + QDateTime::currentDateTime().toString("hh:mm:ss.zzz") + ":" + message);
